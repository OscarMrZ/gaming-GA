...
def get_action(frames, model, print_probs=False):
    ...
    # Ensure all frames are (75, 75) and there are no None elements
    processed_frames = [frame for frame in frames if frame is not None and frame.shape == (75, 75)]

    # If there are less than 4 frames, pad the input with zeros
    while len(processed_frames) < 4:
        processed_frames.append(np.zeros((75, 75)))

    # Stack the frames along the last dimension to form an input of shape (75, 75, 4)
    input_data = np.stack(processed_frames, axis=-1)

    # Ensure the input data is correctly shaped for the model
    input_data = input_data.reshape(1, 75, 75, 4)  # Adding batch dimension
...
def run_game(sleep_time, model_list):
    ...
    while run:
        ...
        for player in player_list:
            prepared_frame = get_prepared_frame((400, 400), (75, 75), background, obstacles, player)
...
def main():
    ...
    model = keras.models.load_model('best_model_interrupted.keras')
    run_game(0, [model])
