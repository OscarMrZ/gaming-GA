...
def get_action(frames, model, print_probs=False):

    num_frames = 4  # Number of frames expected by the model

    # Ensure all frames are of the correct frame size
    processed_frames = [frame for frame in frames if frame is not None and frame.shape == frame_size]

    # If there are less than 4 frames, pad the input with zeros
    while len(processed_frames) < num_frames:
        processed_frames.append(np.zeros(frame_size))

    # Stack the frames along the first dimension to form an input of the correct shape
    input_data = np.stack(processed_frames, axis=0)

    # Ensure the input data is correctly shaped for the model
    input_data = input_data.reshape(1, num_frames, frame_size[0], frame_size[1], 1)  # Adding batch dimension

    # Make the prediction
    probs = model.predict(input_data, verbose=0)

    # Convert the prediction to an action
...
def run_game(sleep_time, model_list):
    ...
    # Initialize a dictionary to store frames for each player
    player_frames = {player: [] for player in player_list}
    ...
    while run:
        ...
        small_margin = int(big_margin * 0.4)
        ...
        for player in player_list:
            prepared_frame = get_prepared_frame((400, 400), frame_size, background, obstacles, player)
            player.frame = prepared_frame

            # Update the player_frames dictionary
            player_frames[player].append(prepared_frame)

            # Ensure only the last 4 frames are stored
            if len(player_frames[player]) > 4:
                player_frames[player].pop(0)

        # Get the control action and execute it
        for player in player_list:
            if player.is_alive:
                # Pass the list of last 4 frames to the network
                action = get_action(player_frames[player], player.brain, False)
        ...
        if current_time - start_time > 180000:  # 120000 milliseconds = 3 minutes
            run = False
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Run game with a specified model.')
