    ...
    while run:
        ...
        if rng in list(range(1, 5)):
            ...
            if n_ground > 0:
                ...
                obs_margin = speed * 40
                if obs_margin > max_obs_margin: obs_margin = max_obs_margin
                
                # Find the last ground obstacle
                last_ground_obstacle = None
                for obs in reversed(obstacles):
                    if obs.type == GROUND:
                        last_ground_obstacle = obs
                        break

                if last_ground_obstacle != None and win_width - last_ground_obstacle.rect.x > obs_margin:
        ...
        # Create new aereal obstacles
        if rng in list(range(10, 12)):

            n_aereal = count_types(obstacles, AEREAL)
            ...
            if n_aereal > 0:
                ...
                # Find the last ground obstacle
                last_aereal_obstacle = None
                for obs in reversed(obstacles):
                    if obs.type == AEREAL:
                        last_aereal_obstacle = obs
                        break

                if last_aereal_obstacle != None and win_width - last_aereal_obstacle.rect.x > obs_margin:
                    new_obs1 = Obstacle(win_width, ground_height - 150, './data/noel.png', obs_size, speed, AEREAL)
                    obstacles.append(new_obs1)
            else:
                new_obs1 = Obstacle(win_width, ground_height - 150, './data/noel.png', obs_size, speed, AEREAL)
                obstacles.append(new_obs1)
